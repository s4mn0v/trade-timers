...
class TimeframeSyncWidget:
    ...
    def on_timer_double_click(self, index):
        if self.armed_timer == index:
            self.armed_timer = None
            self.alert_label.config(text="")
        else:
            self.armed_timer = index
    ...
    def create_widgets(self):
        ...
        for i, tf in enumerate(self.TIMEFRAMES):
            ...
            lbl = tk.Label(f, text=tf["label"], bg='#1a1a1a',
                           fg=tf["color"], font=('Arial', 9, 'bold'))
            lbl.pack()
            ...
            t = tk.Label(t_frame, text="00:00", bg='#1a1a1a',
                         fg='#ffffff', font=('Courier New', 16, 'bold'))
            t.pack(side=tk.LEFT)
            t.bind("<Double-Button-1>",
                   lambda e, idx=i: self.on_timer_double_click(idx))
        ...
        self.alert_label = tk.Label(self.session_row,
                                    text="",
                                    bg='#1a1a1a',
                                    fg='#ff4d4d',
                                    font=('Arial', 10, 'bold'))
        ...
        self.badge = tk.Label(self.session_row, text="---",
                              bg='#333333', fg='#ffffff',
                              font=('Arial', 9, 'bold'),
                              padx=12, pady=3)
        self.badge.pack(side=tk.LEFT, padx=15)

        self.session_timer = tk.Label(self.session_row,
                                      text="--h --m --s",
                                      bg='#1a1a1a',
                                      fg='#ffffff',
    ...
    def update_clock(self):
        ...
        for i, tf in enumerate(self.TIMEFRAMES):
            ...
            if self.armed_timer == i and rem == 1:
                self.play_sound()
                self.armed_timer = None
                self.alert_label.config(text="")
    ...
    def update_session(self):
        ...
        for s in self.SESSIONS:
            ...
            if s_mins < e_mins:
                if s_mins <= cur_m < e_mins:
                    active = s
            else:
                if cur_m >= s_mins or cur_m < e_mins:
                    active = s
